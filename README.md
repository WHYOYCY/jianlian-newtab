...
| 组件 | 说明 |
|------|------|
| **历史记录** | 读取浏览器最近访问，3×2 大卡片；显示条数可选 8/12/20/30；图标与快捷网址共享缓存（不会每个条目各发一次请求） |
...
- **Esc 逐层退出**：编辑弹窗 → 设置弹窗 → 退出编辑模式
- **键盘可用**：Tab 聚焦有清晰焦点环，弹窗内焦点循环、关闭后焦点归位
- **主题**：右上角按钮一键切换白天 / 黑夜；「跟随系统 / 浅色 / 深色」三种模式在设置弹窗里选，选择会被记住

### 🔐 数据可靠
- 配置存本机为主（`storage.local`，5MB），云端镜像用于跨设备同步
- 云端配额不足时**自动降级**为仅本地保存并提示，绝不丢数据
- **配置导出 / 导入**（设置弹窗内）：备份、换电脑迁移、分享给别人
- 不含任何统计、埋点、远程服务器
...
| `Esc` | 关闭编辑弹窗 → 关闭设置弹窗 → 退出编辑模式（逐层） |
| `Enter` / `Space` | 激活当前聚焦的按钮 |
| 编辑模式下拖拽卡片 | 调整顺序 |
| 点击天气卡片 | 立即刷新（重新定位 + 拉取） |
| 点右上角主题按钮 | 白天 ↔ 黑夜 切换（每次点击都会变） |
| 点右上角设置按钮 | 打开设置：主题 + 背景选择 + 配置导出/导入 |
...
├── test/                  # Node 测试套件（无浏览器模拟，42 个用例）
│   ├── helpers.mjs        # chrome API 与 DOM 模拟
│   ├── icons.test.mjs     # 图标服务
│   ├── store.test.mjs     # 存储 / 迁移 / 配额降级
│   ├── widgets.test.mjs   # 组件注册、表单、serialize
│   ├── theme.test.mjs     # 主题切换与持久化
│   └── backup.test.mjs    # 导出 / 导入
└── js/
    ├── core.js            # 核心：存储 / 数据包 / 渲染引擎 / 弹窗 / 主题 / 光效
...
设置弹窗（右上角齿轮图标）里有「导出配置 / 导入配置」，导出的 JSON 可用于备份、换电脑迁移或分享给别人。
也可以直接复制浏览器扩展数据目录，或依赖 `chrome.storage.sync` 的跨设备同步。
</details>
...
- **跑测试**（无需浏览器，用 Node 内置测试运行器）：
  ```bash
  npm test          # 42 个用例：存储迁移、图标缓存、组件、主题、导出导入、文本安全
  ```
- **重新生成图标**（换一张源图时）：
  ```bash
...
| 版本 | 主要变化 |
|------|---------|
| **2.3.1** | 修复城市名与组件标题的 HTML 注入；图标缓存按 host（含端口）区分；天气不再静默回退北京；主题按钮改为白天/黑夜互换（不再有「点了没反应」）、修复有背景图时深色不生效、「跟随系统」移入设置 |
| **2.3.0** | 主题可手动切换（跟随系统/浅色/深色）；配置导出导入；历史图标与快捷网址共享缓存（每页最多省 30 个请求）；引入 35 个用例的测试套件 |
